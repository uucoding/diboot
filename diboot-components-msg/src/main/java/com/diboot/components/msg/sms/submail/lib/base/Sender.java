...
import com.diboot.components.msg.sms.submail.utils.HttpDeleteUtil;
...
public class Sender implements ISender {
	...
	protected boolean putMethodRequest(String url, Map<String, Object> data)  {
		...
		httpput.addHeader("charset", BaseCons.CHARSET_UTF8);
		data.put(APPID, config.getAppId());
		data.put(TIMESTAMP, this.getTimestamp());
		data.put(SIGNATURE, config.getAppKey());
		...
		try {
			httpput.setEntity(new UrlEncodedFormEntity(params, BaseCons.CHARSET_UTF8));
	...
	protected boolean deleteMethodRequest(String url, Map<String, Object> data) {
    	...
    	try{
			...
			if(httpEntity2 != null){
				String jsonStr = EntityUtils.toString(httpEntity2, BaseCons.CHARSET_UTF8);
				return jsonStr.contains("success");
			}
			closeableHttpClient.close();
		}catch(ClientProtocolException e){
