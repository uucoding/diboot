...
public class FileHelper{
	...
	public static String saveImage(File file, String imgName, boolean reserve){
		...
		try{
			...
			if (!reserve && !V.equal(file.getAbsolutePath(), fullPath)){
	...
	public static String saveFile(File file, String fileName, boolean reserve){
		// 生成图片路径
		String accessPath = getImageStoragePath(fileName);
		String fullPath = getFileStorageDirectory() + accessPath;
		try{
			// 创建文件夹
			makeDirectory(fullPath);
	 ...
	 * @return
	 */
	public static String getImageStoragePath(String imageName){
		StringBuilder sb = new StringBuilder();
		sb.append(PATH_IMG).append("/").append(D.getYearMonth()).append("/").append(imageName);
		return sb.toString();
	}
	...
	public static String getAudioStoragePath(String category, String name) {
		...
		sb.append("/").append(D.getYearMonthDay()).append("/").append(name);
		return sb.toString();
	}
	...
	public static String getFileStorageDirectory(){
		if(fileStorageDirectory == null){
			fileStorageDirectory = BaseConfig.getProperty(FILE_STORAGE_DIRECTORY);
		}
	...
	public static void downloadLocalFile(String path, HttpServletRequest request, HttpServletResponse response) throws Exception{
    	...
    	try{
    		...
    		String fileName = new String(outFileName.getBytes(BaseCons.CHARSET_UTF8), "ISO8859-1");
